...
type Evento = { id: number; fecha: string; tipo: "registro" | "ftd"; pais: string | null; ganado: number | null; porBot: boolean };
type Totales = { registros: number; ftd: number; ganado: number };
...
export default function ActividadPage() {
  ...
  const [hoy, setHoy] = useState<Totales>({ registros: 0, ftd: 0, ganado: 0 });
  const [mes, setMes] = useState<Totales>({ registros: 0, ftd: 0, ganado: 0 });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!session) return;
      const res = await fetch("/api/account/actividad", { headers: { Authorization: "Bearer " + session.access_token } });
      if (!res.ok) { setLoadError(true); return; }
      const body = await res.json();
      setEventos(body.eventos ?? []);
      if (body.hoy) setHoy(body.hoy);
      if (body.mes) setMes(body.mes);
    } catch {
      setLoadError(true);
    } finally {
      setLoading(false);
  ...
  return (
    <main className="flex flex-col gap-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          onClick={async () => { setRefrescando(true); try { await load(); } finally { setRefrescando(false); } }}
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="rounded-xl border border-white/15 bg-white/5 p-4">
          ...
          <p className="text-2xl font-bold text-white tabular-nums">{hoy.registros}</p>
          <p className="text-xs text-slate-500 tabular-nums mt-1">{mes.registros} este mes</p>
        </div>
        <div className="rounded-xl border border-white/15 bg-white/5 p-4">
          <p className="text-xs text-slate-400">FTD hoy</p>
          <p className="text-2xl font-bold text-white tabular-nums">{hoy.ftd}</p>
          <p className="text-xs text-slate-500 tabular-nums mt-1">{mes.ftd} este mes</p>
        </div>
        <div className="rounded-xl border border-emerald-400/40 bg-emerald-500/10 p-4">
          ...
          <p className="text-2xl font-bold text-emerald-300 tabular-nums">{eur(hoy.ganado)}</p>
          <p className="text-xs text-slate-400 tabular-nums mt-1">{eur(mes.ganado)} este mes</p>
        </div>
      </div>
      ...
      <div className="bg-white/10 backdrop-blur border border-white/20 rounded-xl overflow-x-auto min-w-0">
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {eventos.length === 0 ? (
              <tr><td colSpan={4} className="border border-white/10 px-4 py-6 text-center text-slate-400">Todavía no hay actividad.</td></tr>
            ) : (
              eventos.map((e, i) => (
                <tr key={e.id} className={`${i % 2 === 1 ? "bg-white/[0.03]" : ""} hover:bg-white/10 transition-colors`}>
                  ...
                  <td className="border border-white/10 px-4 py-3">
                    {e.tipo === "ftd" ? (
                      <span className="inline-flex items-center rounded-full bg-emerald-500/15 text-emerald-300 px-2 py-0.5 text-xs font-semibold">FTD</span>
                    ) : (
                      <span className="inline-flex items-center rounded-full bg-white/10 text-slate-200 px-2 py-0.5 text-xs font-semibold">Registro</span>
                    )}
